...
from us3_module.curated import EVENTS_PAYLOAD_SCHEMA, VALID_STATUSES, dedupe_by_key
from us3_module.datamart import MART_TABLES, reader_grant_statements
...
def test_sandbox_and_shared_names_differ():
    ...
    sandbox = build_context({"env": "nonprod", "use_case": "us3", "schema_prefix": "jsmith_"})
    shared = build_context({"env": "prod", "use_case": "us3"})
    assert sandbox.table("curated", "events") != shared.table("curated", "events")


def test_this_use_case_cannot_collide_with_another():
    a = build_context({"env": "prod", "use_case": "us3"})
...
def test_layers_resolve_to_different_catalogs():
    ctx = build_context({"env": "prod", "use_case": "us3"})
...
def test_payload_schema_declares_every_field_conform_reads():
    """conform_events selects these by name; dropping one would fail at
    runtime with an unresolved-column error, in prod."""
    for field in ("events_id", "event_ts", "status", "amount", "currency"):
        assert f"{field} " in EVENTS_PAYLOAD_SCHEMA


def test_amount_is_typed_not_inferred():
    """Inferring amount from JSON yields a string often enough to matter."""
    assert "amount DOUBLE" in EVENTS_PAYLOAD_SCHEMA


def test_valid_statuses_are_upper_case():
    """conform_events upper-cases before comparing; a lower-case entry here
...
def test_sandbox_marts_are_not_shared():
    ctx = build_context({"env": "nonprod", "use_case": "us3", "schema_prefix": "jsmith_"})
...
def test_prod_grants_reach_business_analysts():
    ...
    assert all(s.startswith("GRANT SELECT ON TABLE edp_datamart_prod.us3.") for s in stmts)


def test_preprod_grants_qa_but_not_business_users():
    ctx = build_context({"env": "preprod", "use_case": "us3"})
