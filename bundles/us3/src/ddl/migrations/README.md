...
`../curated/events.sql` and `../datamart/marts.sql` describe the *current
...
`us3_migrate` refuses to run on a bad name, a duplicate version, a
...
| ✅ | Update `../curated/events.sql` in the **same PR**, so the shape file stays the truth for a fresh environment. |
...
```sql
DROP TABLE IF EXISTS edp_curated_nonprod.jsmith_us3.events;
```
...
```sql
-- V008__add_events_amount_decimal.sql   (release N)
...
-- V011__drop_events_amount_double.sql   (release N+1)
