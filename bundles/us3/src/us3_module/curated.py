"""us3 curated-layer transformations: landing -> curated.
...
# Expected shape of the events payload landed by the shared landing bundle.
# Bumping this is a reviewable, testable change rather than a schema-inference
# surprise at 2am.
EVENTS_PAYLOAD_SCHEMA = (
    "events_id STRING, event_ts STRING, status STRING, amount DOUBLE, currency STRING"
)
...
def conform_events(df):
    """Standardise landed events records into the curated contract.
    ...
    return df.select(
        F.col("events_id").cast("string").alias("events_id"),
