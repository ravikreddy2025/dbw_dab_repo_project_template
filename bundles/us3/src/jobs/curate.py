...
# MAGIC %md
# MAGIC # us3: landing -> curated
# MAGIC
# MAGIC Thin entry point. The transformation itself is
# MAGIC `us3_module.curated.conform_events`, unit-tested in `tests/`.
# MAGIC
# MAGIC **Porting note.** While us3 logic still lives in `src/ported/`, call it
...
from us3_module.curated import conform_events, dedupe_by_key
...
with audited_run(spark, ctx, layer="curated"):
    ...
    curated = dedupe_by_key(curated, keys=["events_id"], order_by="event_ts")

    # NO overwriteSchema. The table is created and evolved by us3_migrate from
    # src/ddl/curated/events.sql and src/ddl/migrations/ - this job writes DATA.
    #
    ...
    record_table_load(
        spark, ctx, source_id="us3_events_curated", target_table=target,
        rows_written=rows, load_strategy="full",
    )
    print(f"{rows:,} events rows written")
...
results = evaluate(
    ...
    [
        not_null("events_id"),
        unique("events_id"),
        # conform_events nulls unrecognised statuses rather than dropping the
