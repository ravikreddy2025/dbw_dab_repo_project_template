...
print(f"publishing us3 marts in {ctx.fq_schema('datamart')}")
...
with audited_run(spark, ctx, layer="datamart"):
    for table in MART_TABLES:
        ...
        record_table_load(
            spark, ctx, source_id=f"us3.{table}", target_table=fq,
...
fct = ctx.table("datamart", "fct_events")
results = evaluate(
    spark,
    ctx,
    fct,
