-- =============================================================================
-- us4 datamart: dim_inventory
-- =============================================================================
-- >>> PLACEHOLDER - replace with the ported us4 mart definition. <<<
--
...
--   sandbox : edp_datamart_nonprod.jsmith_us4.dim_inventory
--   prod    : edp_datamart_prod.us4.dim_inventory
--
...
CREATE TABLE IF NOT EXISTS
  IDENTIFIER(:catalog || '.' || :schema || '.dim_inventory') (
    inventory_key      BIGINT   GENERATED ALWAYS AS IDENTITY,
    inventory_id       STRING   NOT NULL COMMENT 'Natural key from the source system.',
    status            STRING,
    currency          STRING,
...
USING DELTA
COMMENT 'Dimension for us4 inventory. Built by the edp_us4 bundle.';

-- MERGE rather than CREATE OR REPLACE so inventory_key stays stable across runs.
-- A surrogate key that changed nightly would break every saved report using it.
MERGE INTO IDENTIFIER(:catalog || '.' || :schema || '.dim_inventory') AS t
USING (
  SELECT inventory_id, status, currency, event_ts AS source_updated_at
  FROM IDENTIFIER(:curated_catalog || '.' || :schema || '.inventory')
) AS s
...
WHEN NOT MATCHED THEN INSERT (
    inventory_id, status, currency, source_updated_at, dw_updated_at
  ) VALUES (
    s.inventory_id, s.status, s.currency, s.source_updated_at, current_timestamp()
  );
