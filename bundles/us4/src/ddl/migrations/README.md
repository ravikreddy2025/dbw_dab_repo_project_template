...
`../curated/inventory.sql` and `../datamart/marts.sql` describe the *current
...
```
V007__add_inventory_currency.sql
 |      |
...
`us4_migrate` refuses to run on a bad name, a duplicate version, a
...
| ✅ | Update `../curated/inventory.sql` in the **same PR**, so the shape file stays the truth for a fresh environment. |
...
```sql
DROP TABLE IF EXISTS edp_curated_nonprod.jsmith_us4.inventory;
```
...
```sql
-- V008__add_inventory_amount_decimal.sql   (release N)
...
-- V011__drop_inventory_amount_double.sql   (release N+1)
