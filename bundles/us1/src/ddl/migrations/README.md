...
`../curated/orders.sql` and `../datamart/marts.sql` describe the *current
...
`us1_migrate` refuses to run on a bad name, a duplicate version, a
...
| ✅ | Update `../curated/orders.sql` in the **same PR**, so the shape file stays the truth for a fresh environment. |
