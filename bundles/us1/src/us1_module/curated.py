"""us1 curated-layer transformations: landing -> curated.
...
# Expected shape of the orders payload landed by the shared landing bundle.
# Bumping this is a reviewable, testable change rather than a schema-inference
# surprise at 2am.
ORDERS_PAYLOAD_SCHEMA = (
    "orders_id STRING, event_ts STRING, status STRING, amount DOUBLE, currency STRING"
)
...
def conform_orders(df):
    """Standardise landed orders records into the curated contract.
    ...
    return df.select(
        F.col("orders_id").cast("string").alias("orders_id"),
