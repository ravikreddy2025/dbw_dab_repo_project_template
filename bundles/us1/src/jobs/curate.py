...
# MAGIC %md
# MAGIC # us1: landing -> curated
# MAGIC
# MAGIC Thin entry point. The transformation itself is
# MAGIC `us1_module.curated.conform_orders`, unit-tested in `tests/`.
# MAGIC
# MAGIC **Porting note.** While us1 logic still lives in `src/ported/`, call it
...
from us1_module.curated import conform_orders, dedupe_by_key
...
# edp_landing_nonprod.us1, not to an empty jsmith_us1. Chain your own landing
...
target = ctx.table("curated", "orders")
...
with audited_run(spark, ctx, layer="curated"):
    ...
    # >>> PLACEHOLDER: replace with the ported us1 transformation. <<<
    curated = conform_orders(landed)
    # Landing is append-only, so an incremental reload or a stream replay can
    # carry the same record twice. Latest event_ts wins.
    curated = dedupe_by_key(curated, keys=["orders_id"], order_by="event_ts")

    # NO overwriteSchema. The table is created and evolved by us1_migrate from
    # src/ddl/curated/orders.sql and src/ddl/migrations/ - this job writes DATA.
    #
    ...
    record_table_load(
        spark, ctx, source_id="us1_orders_curated", target_table=target,
        rows_written=rows, load_strategy="full",
    )
    print(f"{rows:,} orders rows written")
...
results = evaluate(
    ...
    [
        not_null("orders_id"),
        unique("orders_id"),
        # conform_orders nulls unrecognised statuses rather than dropping the
