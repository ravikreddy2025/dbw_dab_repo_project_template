...
from us1_module.curated import ORDERS_PAYLOAD_SCHEMA, VALID_STATUSES, dedupe_by_key
from us1_module.datamart import MART_TABLES, reader_grant_statements
...
def test_sandbox_and_shared_names_differ():
    ...
    sandbox = build_context({"env": "nonprod", "use_case": "us1", "schema_prefix": "jsmith_"})
    shared = build_context({"env": "prod", "use_case": "us1"})
    assert sandbox.table("curated", "orders") != shared.table("curated", "orders")
...
def test_payload_schema_declares_every_field_conform_reads():
    """conform_orders selects these by name; dropping one would fail at
    runtime with an unresolved-column error, in prod."""
    for field in ("orders_id", "event_ts", "status", "amount", "currency"):
        assert f"{field} " in ORDERS_PAYLOAD_SCHEMA


def test_amount_is_typed_not_inferred():
    """Inferring amount from JSON yields a string often enough to matter."""
    assert "amount DOUBLE" in ORDERS_PAYLOAD_SCHEMA


def test_valid_statuses_are_upper_case():
    """conform_orders upper-cases before comparing; a lower-case entry here
...
def test_prod_grants_reach_business_analysts():
    ...
    assert all(s.startswith("GRANT SELECT ON TABLE edp_datamart_prod.us1.") for s in stmts)
