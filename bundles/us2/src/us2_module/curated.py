"""us2 curated-layer transformations: landing -> curated.
...
# Expected shape of the customers payload landed by the shared landing bundle.
# Bumping this is a reviewable, testable change rather than a schema-inference
# surprise at 2am.
CUSTOMERS_PAYLOAD_SCHEMA = (
    "customers_id STRING, event_ts STRING, status STRING, amount DOUBLE, currency STRING"
)
...
def conform_customers(df):
    """Standardise landed customers records into the curated contract.
    ...
    return df.select(
        F.col("customers_id").cast("string").alias("customers_id"),
