...
def test_payload_schema_declares_every_field_conform_reads():
    """conform_customers selects these by name; dropping one would fail at
    runtime with an unresolved-column error, in prod."""
    for field in ("customers_id", "event_ts", "status", "amount", "currency"):
        assert f"{field} " in CUSTOMERS_PAYLOAD_SCHEMA


def test_amount_is_typed_not_inferred():
    """Inferring amount from JSON yields a string often enough to matter."""
    assert "amount DOUBLE" in CUSTOMERS_PAYLOAD_SCHEMA


def test_valid_statuses_are_upper_case():
    """conform_customers upper-cases before comparing; a lower-case entry here
