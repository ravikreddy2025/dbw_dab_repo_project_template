...
from us5_module.datamart import MART_TABLES, reader_grant_statements
...
def test_sandbox_and_shared_names_differ():
    ...
    sandbox = build_context({"env": "nonprod", "use_case": "us5", "schema_prefix": "jsmith_"})
    shared = build_context({"env": "prod", "use_case": "us5"})
    assert sandbox.table("curated", "settlement") != shared.table("curated", "settlement")
...
def test_sandbox_marts_are_not_shared():
    ctx = build_context({"env": "nonprod", "use_case": "us5", "schema_prefix": "jsmith_"})
...
def test_prod_grants_reach_business_analysts():
    ...
    assert all(s.startswith("GRANT SELECT ON TABLE edp_datamart_prod.us5.") for s in stmts)


def test_preprod_grants_qa_but_not_business_users():
    ctx = build_context({"env": "preprod", "use_case": "us5"})
