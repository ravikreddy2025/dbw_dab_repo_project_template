...
`../curated/settlement.sql` and `../datamart/marts.sql` describe the *current
...
`us5_migrate` refuses to run on a bad name, a duplicate version, a
...
| ✅ | Update `../curated/settlement.sql` in the **same PR**, so the shape file stays the truth for a fresh environment. |
...
```sql
DROP TABLE IF EXISTS edp_curated_nonprod.jsmith_us5.settlement;
```
...
```sql
-- V008__add_settlement_amount_decimal.sql   (release N)
...
-- V011__drop_settlement_amount_double.sql   (release N+1)
